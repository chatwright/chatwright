...
| I-71 | Portable scenario format: the declarative representation Recipes, demos and the browser runtime share (relationship to Starlark plans, specscore-rehearse-verification and Go fragments). Founder requirement (2026-07-23): deterministic user actions and expectations must be scriptable without Go — e.g. "expect a bot message with exactly buttons [Yes, No]; user clicks [Yes]" — mirroring the `cw` verb vocabulary (SendText/Click/ExpectBotMessage/ExpectActions/ExpectEdited/Within), executable by the browser runtime. | Format proposal with three real scenarios expressed in it and executed by at least one runtime; the yes/no button example above must be among them. |
