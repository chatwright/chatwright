...
|---|---|---|
| I-66 | **Design-only advancement 2026-07-30: see [browser-endpoint-profile](../ideas/browser-endpoint-profile.md) — a distinct concept filed under the same ID for lack of a better anchor (see that document's Context): a new declared `browser` endpoint profile (decision 0008), driving a real, external browser against a real web application via `playwright-go`, with the ChessRaiders two-browser signed-in match (`sneat-co/chess`, shipping now on Playwright directly, unblocked by this design) as its acceptance workload. This does not resolve the question below, which stays open.** Browser runtime internals: how does runtime-ts structure orchestration (scenario execution, bot registry, routing, correlation, state) inside the Playground/player component, and how does the player's settled-fold rendering accept a live, append-only journal instead of a finished bundle? | Component/state architecture spec; an `appendStep`-style engine contract the existing player components can consume; an embedding contract for demos on any page. |
...
| I-71 | **Specified as [self-contained-scenario-documents](../features/chatwright/scenario-authoring/portable-scenario-documents/self-contained-scenario-documents/README.md) (founder decision, 2026-07-25: a self-contained document, not scenarios registered in code); the AI-goal half is covered there, the deterministic half stays open pending action matchers.** Portable scenario format: the declarative representation Recipes, demos and the browser runtime share (relationship to Starlark plans, specscore-rehearse-verification and Go fragments). Founder requirement (2026-07-23): deterministic user actions and expectations must be scriptable without Go — e.g. "expect a bot message with exactly buttons [Yes, No]; user clicks [Yes]" — mirroring the `cw` verb vocabulary (SendText/Click/ExpectBotMessage/ExpectActions/ExpectEdited/Within), executable by BOTH runtimes — browser (TypeScript) and CLI (Go) — with identical semantics; one scenario file, two runtimes, same verdict. | Format proposal with three real scenarios expressed in it and executed by both runtimes; the yes/no button example above must be among them, passing identically in each. |
...
| I-77 | Recipe code snippets at scale. Founder feature (2026-07-23): every Recipe page carries a "Code" card with per-framework tabs (popular bot frameworks + vanilla per language). Predefined snippet files shipped first (`recipes/<id>/snippets/<framework>.md` convention, exemplar: language-onboarding with vanilla JS / bots-fw Go / python-telegram-bot); later snippets are auto-generated — deterministically from a scenario/recipe model where possible, AI-assisted where not — and must stay idiomatic and verified against each framework's real API. | Generation design (deterministic vs AI-assisted split, verification gate per framework, drift policy when frameworks change); three generated snippets matching the hand-written exemplars. |
