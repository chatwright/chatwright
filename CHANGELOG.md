...
- New `actor/openai` package: an `actor.Provider` for any OpenAI-compatible `/chat/completions` server (Ollama, LM Studio, OpenRouter, vLLM, OpenAI itself) — one provider covers all of them because they share the wire shape, differing only by `Config.BaseURL` and, for hosted services, `Config.APIKey` (optional; sent as `Authorization: Bearer` only when set). Renders the SAME proposal JSON contract `actor/anthropic` enforces (`send-text` | `click` | `task-done` | `give-up` + rationale, one repair attempt, a typed `InvalidResponseError` on anything that still fails to parse or violates the contract — never a fabricated `Proposal`) via `response_format: {"type":"json_schema", strict:true}`; on any rejection other than authentication (401/403) or rate-limit (429) — the design brief's "400/unknown" — it retries the same prompt exactly once with `response_format: {"type":"json_object"}` and the schema restated in the system prompt, surfacing a typed `FallbackFailedError` if that also fails, and exposes which mode served the last call via `Provider.LastResponseFormatMode`. `Usage.Model`/`InputTokens`/`OutputTokens` come from the response (falling back to `Config.Model`, or zero when the server omits `usage`); `Usage.Cost` is always left nil — unlike `actor/anthropic`'s dated pricing snapshot, there is no shared price list across arbitrary local/self-hosted servers. Composes with `actor.CassetteProvider` like any other `Provider`; tests run entirely against a fake `httptest.Server` at zero network cost, including a greetbot campaign driven end to end through the provider whose assembled bundle validates against `formats/run-bundle/v1/schema.json`, plus one optional live-local-LLM smoke test gated behind `CHATWRIGHT_LIVE_LOCAL_LLM=1` and `CHATWRIGHT_LOCAL_LLM_BASE_URL`. See `actor/openai/README.md`.
