...
- `campaign.Bundle`: the persisted, self-contained campaign run bundle a Web UI player (Chatwright Studio) replays — versioned (`BundleSchemaVersion`), JSON-tagged, carrying the `Goal`, every chat's `platform.JournalEntry` history, the actor's retained `observe.Observation` bodies, every `actor.LoopEvent`, the assembled `Report`, optional `datastate.Evidence` and a `Metadata` block (caller-supplied `CreatedAt`, the chatwright module version via `runtime/debug` when available, platform name, declared endpoint profile, aggregated provider/model ids) — deliberately excludes provider-layer `actor.Cassette`s (a separate, reusable artifact; see Bundle's godoc). `WriteBundle`/`ReadBundle` give deterministic, human-readable, indented JSON I/O; `ReadBundle` rejects an unrecognised `schemaVersion` with a typed `ErrUnknownBundleSchemaVersion` rather than silently misparsing it. `actor.Config.DisableObservationRetention` (default off, i.e. retention on) plus `actor.Loop.Observations` supply the retained observation bodies a Bundle needs.
