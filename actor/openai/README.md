...
[`spec/ideas/openai-compatible-provider.md`](../../spec/ideas/openai-compatible-provider.md).

## Quick start

```go
import "github.com/chatwright/chatwright/actor/openai"
...
- `go test ./actor/openai/...` runs the full suite — request shape, all
  ...
  validates against `formats/run-bundle/v1/schema.json` — entirely against
